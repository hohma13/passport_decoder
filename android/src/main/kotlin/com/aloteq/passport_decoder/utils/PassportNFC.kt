...
private constructor() {
    ...
    constructor(
        ...
        try {
            ...
            if (hasSAC) {
                try {
                    paceResult = doPACE(ps, documentNumber, dateOfBirth, dateOfExpiry)
                    isSACSucceeded = true
                } catch (e: Exception) {
                    e.printStackTrace()
        ...
        if (hasBAC && !(hasSAC && isSACSucceeded)) {
            ...
            try {
                doBAC(service as PassportService, documentNumber,
                        dateOfBirth,
                        dateOfExpiry)
        ...
        if (hasCA) {
            try {
                val eaccaResults = doEACCA(ps, documentNumber,
                        dateOfBirth,
                        dateOfExpiry, dg14File, sodFile)
        ...
        if (hasEAC && cvcaKeyStores != null && cvcaKeyStores.size > 0 && verificationStatus.ca == VerificationStatus.Verdict.SUCCEEDED) {
            try {
                val eactaResults = doEACTA(ps, documentNumber,
                        dateOfBirth,
                        dateOfExpiry, cvcaFile, paceResult, verificationStatus.caResult, cvcaKeyStores)
    ...
    @Throws(CardServiceException::class)
    private fun doBAC(ps: PassportService, documentNumber: String?,
                      dateOfBirth: String?,
                      dateOfExpiry: String?): BACResult {
        val bacKey = BACKey(documentNumber, dateOfBirth, dateOfExpiry)
        return ps.doBAC(bacKey)
    }


    private fun doEACCA(ps: PassportService, documentNumber: String?,
                        dateOfBirth: String?,
                        dateOfExpiry: String?, dg14File: DG14File?, sodFile: SODFile?): List<EACCAResult> {
        if (dg14File == null) {
    ...
    private fun doEACTA(
        ...
        for (caReference in possibleCVCAReferences) {
            ...
            try {
                if (paceResult == null) {
                    val eactaResult = ps.doEACTA(caReference, terminalCerts, privateKey, null, eaccaResult, documentNumber)
